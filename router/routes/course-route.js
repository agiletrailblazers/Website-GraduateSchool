...
router.get('/courses/:course_id_or_code', function(req, res, next){
  ...
  var courseData = {};
  var content;
  var courseOnlineTypes = ['Instructor-Supported WBT','Web Based Training','Video On Demand','Distance Education','Paper'];
    ...
    if (courseData && courseData.class) {
      ...
      if (courseData.class.type.indexOf(classTypes.evening) > -1) {
          ...
          courseData.session.forEach(function(session) {
            ...
            if ( epRegDeadline.isBefore(currTime) ||
                 epRegDeadline.isSame(currTime)){
              session.status = 'C';
